...
这里的替换不是让你去替换3Dmigoto文件夹里的dll，而是替换SSMT-Package里对应的文件夹。

解压后得到这些东西：

![alt text](image-17.png)

然后直接去我们SSMT-Package的3Dmigoto-GameMod-Fork里替换这俩文件夹：
...
- FPS解锁 (本质上属于作弊软件)
- 超广角解锁 (本质上属于作弊软件)
- d3dxSkinManager(100%重点打击的Mod管理器) 现在已经严重到，只要你开启了d3dxSkinManager，再玩原神Mod就会100%报错，这玩意已经加入进程检测的黑名单了，在任何情况下都不要用这个软件。
