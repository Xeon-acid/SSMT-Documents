...
在开始使用SSMT之前，重启电脑，并且不要开启XXMI、d3dxSkinManager等在黑名单中的工具。
...
- 理论上新版本防报错无需替换d3d11.dll文件，各位如果仍出现错误可以排查一下其它原因，因为现在会扫全盘和进程，建议全新电脑进行测试。
...
- 胡桃工具箱 (本质上属于作弊软件) （但是测试发现Better GI仍然能用，所以可以用Better GI代替胡桃工具箱，估计是还没加入黑名单）
